...
Config (env):
  ...
  XCOIN_EXPLORER_PUBLIC 1 to enable the analytics tag and the forum chat embed
                        ...
                        live site's numbers)
"""
import base64, hashlib, html, json, os, re, threading, time, urllib.request, urllib.error
...
IDENTICON = "https://xcoin-identicon.winnertakeall-17.workers.dev/api/svg?a="
def ico(addr, cls="ico"):
    u = IDENTICON + esc(addr)
    return f'<span class="ico-wrap"><img class="{cls}" src="{u}" alt="" loading="lazy"><img class="ico-pop" src="{u}" alt=""></span>'
_ADDR_LINK = re.compile(r'<a (?!class="hash ico-done")(class="hash" )?href="/address/([a-z0-9]+)">')
_MINER_CELL = re.compile(r'<div class="miner-cell">((?:<b>.*?</b>)?)<a class="hash" href="/address/([a-z0-9]+)">(.*?)</a></div>')
...
CSS = """
.ico{width:1.2em;height:1.2em;vertical-align:-.25em;margin-right:.35em;image-rendering:pixelated}
.ico-md{width:44px;height:44px;flex:none;image-rendering:pixelated}
.ico-big{width:96px;height:96px;display:block;margin-bottom:12px}
.ico-wrap{position:relative;display:inline-block;vertical-align:middle;line-height:0}
.ico-pop{display:none;position:fixed;width:192px;height:192px;image-rendering:pixelated;border:2px solid var(--line);background:var(--paper);box-shadow:0 8px 24px rgba(0,0,0,.35);z-index:50;pointer-events:none}
.ico-wrap:hover .ico-pop{display:block}
...
.miner-cell{display:flex;align-items:center;gap:10px;min-width:0}
.miner-txt{display:flex;flex-direction:column;gap:2px;min-width:0}
...
<style>{CSS}</style><script>document.addEventListener('mouseover',function(e){{var w=e.target.closest&&e.target.closest('.ico-wrap');if(!w)return;var p=w.querySelector('.ico-pop'),r=w.getBoundingClientRect(),S=196;var x=r.right+8,y=r.top+r.height/2-S/2;if(x+S>innerWidth)x=r.left-S-8;y=Math.max(8,Math.min(y,innerHeight-S-8));p.style.left=x+'px';p.style.top=y+'px';}});</script></head><body>
<header class="topbar">
...
<section class="note-panel head-panel section">
  ...
  {ico(addr,"ico-big")}<h1 class="h-small">{a}</h1>
...
class H(BaseHTTPRequestHandler):
    ...
    def _send(self, body, code=200, ctype="text/html; charset=utf-8"):
        if isinstance(body, str) and ctype.startswith("text/html"): body = _MINER_CELL.sub(lambda m: f'<div class="miner-cell">{ico(m.group(2),"ico-md")}<div class="miner-txt">{m.group(1)}<a class="hash ico-done" href="/address/{m.group(2)}">{m.group(3)}</a></div></div>', body)
        if isinstance(body, str) and ctype.startswith("text/html"): body = _ADDR_LINK.sub(lambda m: ico(m.group(2)) + m.group(0), body)
